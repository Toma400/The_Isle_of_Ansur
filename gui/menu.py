...
''')
    ...
    print (align(system.settings.version_call("game_version") + "\n\n", "right"))
    print (align("--------------------", "centre"))
...
def game_load():
  ...
  for i in loaded_profiles:
    if utils.repo_manag.empty_checker("saves/" + i + "/in_use/profile.json") == False:
      ...
      print (align("[ " + utils.colours.bcolors.OKCYAN + i + utils.colours.bcolors.ENDC + " ][" + race + " - " + classe + "][ " + utils.colours.bcolors.OKCYAN + locate + utils.colours.bcolors.ENDC + " ]", "centre_colour"))
      profile_set[str(j)] = i
      j = j + 1
    else:
      pass
...
def settings():
  ...
  for i in setting_options:
    ...
    print (align("[" + str(j) + "][" + i + "][ " + utils.colours.bcolors.OKCYAN + str(y).capitalize() + utils.colours.bcolors.ENDC + " ]", "centre_colour"))
...
def encyclopaedia():
  ...
  for i in range (len(elements)):
    ...
    if i == 1:
      assigned_var = align(utils.colours.bcolors.CRED + "[" + str(i+1) + "] " + elements[i] + utils.colours.bcolors.ENDC, "centre_colour")
    else:
