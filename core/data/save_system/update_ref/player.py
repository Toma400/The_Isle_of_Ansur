...
def updatePlayer(name: str, data: dict = None):
    ...
    if data is not None:
        ...
        # statistics
        player_data["hp_max"]     = statScript(name, SV_KIND.BUFFER.value, init["hp_max"])
        player_data["mp_max"]     = statScript(name, SV_KIND.BUFFER.value, init["mp_max"])
        player_data["sp_max"]     = statScript(name, SV_KIND.BUFFER.value, init["sp_max"])
        player_data["weight_max"] = statScript(name, SV_KIND.BUFFER.value, init["weight_max"])
        player_data["hunger_max"] = statScript(name, SV_KIND.BUFFER.value, init["hunger_max"])
        player_data["thirst_max"] = statScript(name, SV_KIND.BUFFER.value, init["thirst_max"])
